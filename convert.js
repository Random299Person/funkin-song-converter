...
function loadFile(file) {
  ...
  reader.onload = (e) => {
    ...
    function addEvent(timing, value) {
        if (!eventsList[timing]) {
            eventsList[timing] = [value];
        } else {
            eventsList[timing].push(value);
        }
    }
    ...
    if (file.name == "events.json") {
		for (i = 0; i < json.song.notes.length; i++) {
			...
			for (x = 0; x < section.sectionNotes.length; x++) {
				...
				if (olderPhych != null) {
					if (note[1] < 0) {
						addEvent(timing, `${note[2]}_${note[3]}_${note[4]}`);
					}
				} else {
					addEvent(timing, `${note[1][0][0]}_${note[1][0][1]}_${note[1][0][2]}`);
				}
			}
		}
	} else {
			...
			for (x = 0; x < section.sectionNotes.length; x++) {
				...
				if (olderPhych != null) {
					if (note[1] < 0) {
						...
						if (noteHandlingOption != "events_only") {
	...
	if (json.song.events != null) {
		for (i = 0; i < json.song.events.length; i++) {
			// sections
			let note = json.song.events[i];
			let timing = note[0].toFixed(4).padStart(12, '0')
    ...
    lookupArray = lookupArray.concat(Object.keys(eventsList)) // concat with section timings
    ...
    for (var i = 0; i < lookupArray.length; i++) {
        ...
        // if a valid event is at this lookup, add it to the scratch list
        if (Object.keys(eventsList).includes(lookupArray[i])) {
            for (var x = 0; x < eventsList[lookupArray[i]].length; x++) {
                scratchList.push(`!${lookupArray[i]}_${eventsList[lookupArray[i]][x]}`)
            }
        }
    }
